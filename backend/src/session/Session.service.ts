...
import { DatadogLogger } from 'src/logging';
import { SessionTokenDTO } from './dto/Session.dto';
import { SessionDAO } from './schema';
...
export class SessionService {
    ...
    public async deleteSession(sessionId: string) {
        ...
        await this.sessionDAO.deleteOne({ sessionId });

        this.Logger.log(`Session deleted: ${sessionId}`);
    }

    public async updateSessionLastSeen(sessionId: string) {
        this.Logger.debug(`Updating session ${sessionId} last seen`);
